"""
 d_comp.py, continued from ../issue8/db_comp.py
...
def init_db(lines,opt):
 ...
 for iline,line in enumerate(lines):
  ...
  if line.startswith('<LEND>'):
   ...
   group = None
   continue
  for im,m in enumerate(re.finditer(regex,line)):
   dtext = m.group(1)
   #dwords = dtext.split(' ')
   dwords = re.split(regsplit,dtext)
   for db in dwords:
    if db != '▪':
     dbrec = (iline,db)
...
if __name__=="__main__":
 ...
 groups1 = init_db(lines1,splitopt)  
 print(f'{len(groups1)} entries from {filein1}')
 groups2 = init_db(lines2,splitopt)
